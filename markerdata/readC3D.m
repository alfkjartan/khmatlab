...
% AnalogSignals      Analog signals [NanalogSamples x Nsignals ]
...
% Ver. 2.0 Revision (Jaap Harlaar, Amsterdam, april 2002)
  ...
  fid=fopen(FullFileName,'rb','n'); % native format (PC-intel)
  ...
  if key~=80,
    error(['File: ',FileName,' does not comply to the C3D format']);
    fclose(fid)
    return
  end

  % jump to processortype - field
  ...
  Ncharacters=fread(fid,1,'int8');   			
  % characters in group/parameter
  % name
  GroupNumber=fread(fid,1,'int8');
  % % id number -ve=group / +ve=parameter 
  
  try
    while Ncharacters > 0 
      % The end of the parameter record is indicated by <0 characters
      % for group/parameter name 
	...
	ParameterGroup(GroupNumber).description=...
	    cellstr(char(GroupDescription)); %group description
	...
	ParameterNumber=...
	    ParameterNumberIndex(GroupNumber); 
	%index all parameters within a group
	
	ParameterName=fread(fid,[1,Ncharacters],'char');
	% name of parameter
	%char(ParameterName)
	% read parameter name
	...
	if type==-1 %datatype=='char'  
	  ...
	  if dimnum==2 & datalength>0 %& parameter(idnumber,index,2).dim>0            
	    for j=1:dimension(2)
	      ...
	      ParameterGroup(GroupNumber) ...
		  .Parameter(ParameterNumber).data(j)=cellstr(char(data));
	    end                
	  elseif dimnum==1 & datalength>0
	    data=fread(fid,[1,wordlength],'char');
	    %%numerical data record of 1-D array
	    ParameterGroup(GroupNumber) ...
		.Parameter(ParameterNumber).data=cellstr(char(data));
	  end
	...
	deschars=fread(fid,1,'int8');							%description characters        if deschars>0
	...
	fseek(fid,nextrec,'bof');
      ...
      filepos=ftell(fid);		%present file position
      Ncharacters=fread(fid,1,'int8');
      % characters in next group/parameter name
      GroupNumber=fread(fid,1,'int8');
      % id number -ve=group / +ve=parameter
    end
  catch
    % Skip reading more parameters if error occurs. Hope for the
    % best
  end
...
if Scale < 0
%  try
  MarkersAnalog = reshape(fread(fid,NvideoFrames*dataPerFrame,'float32'),...
			  dataPerFrame, NvideoFrames);
%  catch
%    keyboard
%  end
  
  Markers = reshape(MarkersAnalog(1:4*Nmarkers,:), ...
		    4, Nmarkers, NvideoFrames);
  CamRes = uint16(squeeze(Markers(4,:,:))');
  Markers = permute(Markers(1:3,:,:), [3 2 1]);
  hibyte = bitshift(CamRes,-8);
  lobyte = bitshift(CamRes,8);
  lobyte = bitshift(lobyte,-8);
  CameraInfo = double(uint8(hibyte));
  ResidualError = double(lobyte)*abs(Scale);
  nodataind = find(hibyte==255);
  ResidualError(nodataind) = -1;
  
  
  AnalogSignals = (reshape(MarkersAnalog(4*Nmarkers+1:end,:), ...
			   NanalogChannels, ...
			   NanalogFramesPerVideoFrame *...
			   NvideoFrames))';
  
else
  MarkersAnalog = reshape(fread(fid,NvideoFrames*dataPerFrame,'int16'),...
			  dataPerFrame, NvideoFrames);
  Markers = reshape(MarkersAnalog(1:4*Nmarkers,:), ...
		    4, Nmarkers, NvideoFrames);
  CamRes = fix(squeeze(Markers(4,:,:)))';
  Markers = permute(Markers(1:3,:,:), [3 2 1]) * Scale;
  CameraInfo = fix(CamRes/256);
  ResidualError = (CamRes - CameraInfo*256);
