...
%%
 ...
 if (nargin == 0)
   ...
   if fixp12
     Q = diag( cat(2, [1 1 1 10 10 10]*(pi/180)^2, ...
		   [1e1 1e1 1e1], ...
		   [1e-2 1e-2 1e-2]*(pi/180)^2));
     P = Q*10;
     P(10:12,10:12) = eye(3); %% Covariance of initial guess of
   ...
   sd_w = 4 * pi /180 ; % in rad/s
   R = diag(cat(2, ones(1,3)*(pi/180)^2, 0.1*ones(1,3), ones(1,3)*(pi/180)^2, 10*ones(1,3)));
...
function do_unit_test()
  ...
  Nfr = size(wb1,2);
  complete_revolutions = (N-1:N:Nfr);
...
%%  end
  ...
  errrevolB = zeros(cycles*2,1);
  errrevolA = zeros(cycles*2,1);



  for rep = 1: nreps
    p12 = [r2-r;0;0]*(1+randn(1)*0.05); % Apriori estimate of pos of sensor 2 wrt sensor 1
...
%  toc()
  ...
  q0B = [0 0 0 1];
  %%keyboard
  for i = 1:length(complete_revolutions)
    ...
    errrevolB(i) = errrevolB(i) + 1/rep*(abs(eqa) - errrevolB(i));
    q0B = qinv(qB(:,endr));
    ...
    errrevolA(i) = errrevolA(i) + 1/rep*(abs(eqa) - errrevolA(i));
    q0A = qinv(qA(:,endr));
  end
...
%  keyboard
  ...
  print("q12errorABsa.tex", "-depslatexstandalone")
  ...
  print("qerrorABsa.tex", "-depslatexstandalone")
  ...
  print("w_rmseAB.tex", "-depslatexstandalone")
  ...
  print("qerrABsa.tex", "-depslatexstandalone")
