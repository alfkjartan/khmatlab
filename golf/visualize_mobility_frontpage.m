function visualize_mobility_frontpage(WW, colors, clubpath, figs, fnames)
...
for i = 1:nmob
    ...
    if i==1
      axes2 = axes('XAxisLocation','top',...
		   'YAxisLocation','right',...
		   'Color','none', 'FontSize', fszax);
    end
    line(eeyz(1,:), eeyz(2,:), 'Parent', axes2,'color', color, 'linewidth', 3)
    hold on
    %title('Frontal plane')
    xlabel('y [kg^{-1}]')
    ylabel('z [kg^{-1}]')

    set(axes2, 'xlim', axlim2_x)
    set(axes2, 'ylim', axlim2_y)
    axis equal
    axis off
