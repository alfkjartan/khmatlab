...
for fp=usefps
  ...
  for tr=usetrials
    ...
    gm = build_test_model(refdata,mdata);
    ...
    %[msim,simnames, jcs] = sim_model(gm, states, 'jcs');
     ...
     endpoint = extractmarkers(mdata, 'Upper_Torso_1');
     ...
     hold on
     plot(timev, model_endpoint_vel_magn, 'r')
     plot(timev, sum(ang_contribs), 'm')
     
     legend('Raw end point vel', 'Model end point vel', ...
	    'Sum of contrib', 'Location', 'Best')
     ylabel('Velocity [m/s]')
     xlabel('Frames')
    title('Endpoint velocity, using Upper_Torso_1 as endpoint',...
	   'Interpreter', 'None')
