...
for fp=usefps
  ...
  for tr=usetrials
    ...
    [statesbothleft, dataframesbothleft] = track_golf_model(gmbothleft, mdata, filterbandwidth);
    [statesbothright, dataframesbothright] = track_golf_model(gmbothright, mdata, filterbandwidth);
    ...
    [msimright, objdright, objdright2] = simulate_and_plot(gmright, statesright, dataframesright, mdata, ...
					       markers2plot, debug);
    [msimboth, objdboth, objdboth2] = simulate_and_plot(gmbothright, statesbothright, ...
							 dataframesbothright, mdata, ...
							 markers2plot, debug);

    if debug
      convert_radians = 1;
      plotangles(statesbothleft, gmbothleft.gcnames(:,1), angles2plot, convert_radians);
...
%      plotmarkers(objdboth(:,1:3), clubheadfig(3,1), objdboth(:,4:6),clubheadfig(3,1), clubheadfig(3,:), {'left', 'right'});
    ...
    nstsboth = size(statesbothleft, 1);
    ...
    [Wbothleft, Wepbothleft, Mbothleft, Mepbothleft, Mfbothleft, Jhleft, Jhepleft] = ...
        golfer_mobility(gmbothleft, statesbothleft(1:nstsboth/2,:));
    [Wbothright, Wepbothright, Mbothright, Mepbothright, Mfbothright, Jhright, Jhepright] = ...
        golfer_mobility(gmbothright, statesbothright(1:nstsboth/2,:));
	...
	lstfr = min(210, size(Wbothleft,3));
	subplot(311)
