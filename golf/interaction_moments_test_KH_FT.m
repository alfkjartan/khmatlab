...
for fp=usefps
  ...
  for tr=usetrials
    ...
    shoulderabdind = find(ismember(gmleft.gcnames(:,1), 'left shoulder abduction'));
    ...
    % And constant 90 degrees in shoulder abduction
    statesleft(shoulderabdind, :) = pi/2;
    ...
    [msimleft, simnamesleft, localframesleft, framesnamesleft] = ...
        sim_model(gmleft, statesleft(1:nstsleft/2,:), 'localframe');
    ...
    % the trunk rotation axis (vertical). The flex axis is vertical. 
    % x points forwards and y to the left
    e_z = flexaxis;
    e_x = [1,0,0]';
    e_x = e_x - (e_z'*e_x)*e_z;
    e_x = e_x / norm(e_x);
    ...
    nfrs = size(statesleft, 2);
     ...
     tauLArm2 = IMleft(elbowflexind, :)' ...
     + reshape(Mleft(elbowflexind, elbowflexind, :), [N,1]).*angacc'; 
