...
if debug
   ...
   clubheadfig = {'clubhead both', figure('Name', 'Clubhead both')
		  'clubhead left right separate', figure('Name', 'Clubhead separate')};

   mobfig = figure('Name', 'mobility');
...
for fp=usefps
  ...
  for tr=usetrials
    ...
    [statesclub, dataframesclub] = track_golf_model(gmclub, mdata, filterbandwidth);
    ...
    [msimclub, simnamesclub, objdclub, objnamesclub] = ...
     ...
     if debug
       ...
       plotmarkers(objdboth(:,1:3), clubheadfig(1,1), objdboth(:,4:6),clubheadfig(1,1), clubheadfig(1,:), {'left', 'right'});
       plotmarkers(objdleft(:,1:3), clubheadfig(2,1), objdright(:,1:3),clubheadfig(2,1), clubheadfig(2,:), {'left', 'right'});
     
       
     end

     [Wboth, Wepboth, Mboth, Mepboth] = ...
        golfer_mobility_w_club(gmboth, statesboth(1:nstsboth/2,:), gmclub, statesclub(1:6,:)); 
     [Wleft, Wepleft, Mleft, Mepleft] = ...
        golfer_mobility_w_club(gmleft, statesleft(1:nstsleft/2,:), gmclub, statesclub(1:6,:)); 
     [Wright, Wepright, Mright, Mepright] = ...
        golfer_mobility_w_club(gmright, statesright(1:nstsright/2,:), gmclub, statesclub(1:6,:)); 
	...
	subplot(311)
	plot(Wepboth(1,1,1:210)(:));
	hold on
	plot(Wepright(1,1,1:210)(:), 'color', [0 0.8 0]);
	plot(Wepleft(1,1,1:210)(:), 'color', [0.8 0. 0]);
	subplot(312)
	plot(Wepboth(2,2,1:210)(:));
	hold on
	plot(Wepright(1,2,1:210)(:), 'color', [0 0.8 0]);
	plot(Wepleft(1,2,1:210)(:), 'color', [0.8 0. 0]);
	subplot(313)
	plot(Wepboth(3,3,:)(1:210));
	hold on
	plot(Wepright(1,3,1:210)(:), 'color', [0 0.8 0]);
	plot(Wepleft(1,3,1:210)(:), 'color', [0.8 0. 0]);
     end
