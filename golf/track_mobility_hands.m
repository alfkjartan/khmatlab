...
% build_golf_model.m file.
 angles2plot = {'left elbow flexion'
	       'right elbow flexion'};
...
if plot_mobility
   mobfig = [figure('Name', 'Endpoint mobility'), figure('name', 'Endlink mobility'), ...
	    figure('Name', 'Endpoint mobility at impact')];
end
...
mobility_along_path_at_impact_left = zeros(length(usefps), length(usetrials));
mobility_normal_to_path_at_impact_left = zeros(length(usefps), length(usetrials));
mobility_along_path_at_impact_right = zeros(length(usefps), length(usetrials));
mobility_normal_to_path_at_impact_right = zeros(length(usefps), length(usetrials));

mobility_along_path_at_impact_both = zeros(length(usefps), length(usetrials));
mobility_normal_to_path_at_impact_both = zeros(length(usefps), length(usetrials));
...
for fp=usefps
  ...
  for tr=usetrials
    ...
    if isempty(mdata)
       error(['Could not find file ', filestr'])
    end
    ...
    mkdir(ndir);
    disp(['Writing results to folder: ', ndir])
    ...
    if useCustomStartFrame
        ...
        %%mdata{2} = mdata{2}(startfr:startfr+20,:);
    ...
    [msimleft, objdleft, objdleft2] = simulate_and_plot(gmleft, statesleft, dataframesleft, ...
...
%      plotmarkers(objdboth(:,1:3), clubheadfig(3,1), objdboth(:,4:6),clubheadfig(3,1), clubheadfig(3,:), {'left', 'right'});
      ...
      %% Plot clubhead center for both hands, to see how well they coincide
      plotmarkers(objdleft2(:,1:3), clubheadfig(1,1), objdright2(:,1:3),clubheadfig(1,1), clubheadfig(1,:), {'left', 'right'});
      plotmarkers(objdboth2(:,1:3), clubheadfig(3,1), objdboth2(:,4:6),clubheadfig(3,1), clubheadfig(3,:), {'left', 'right'});
