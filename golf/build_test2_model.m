function [tmleft, tmright, endpointleft, endpointright] = build_test2_model(refdata,trialdata)
%  gm = build_golf_model(refdata, trialdata)
%
% Returns two kinematic models for debugging the golf tracking program.
% The models are the left and right side containing segment:
%    pelvis, trunk, upperarm, underarm, hand [, club]
...
%                  from visual3D
% Output
%  tmleft,tmright  <-  struct. Contains the fields 
...
% Kjartan Halvorsen
% 2010-03-21 Besed on build_golf_model from 2009-06-26
...
lhand.CoM = hand_1_l; % Used to compute end point velocity
endpointleft = 'L_Hand_1';
...
rhand.CoM = hand_1_r; % Used to compute end point velocity
endpointright = 'R_Hand_1';
...
[tws_la, p0_la, gcnames_la, jc_la, segmnames_la, CoM_la] = ...
    build_model(luarm, llarm, lhand);
[tws_ra, p0_ra, gcnames_ra, jc_ra, segmnames_ra, CoM_ra] = ...
    build_model(ruarm, rlarm, rhand);
...
gcnames_ub_l = cat(1, gcnames_ub, gcnames_la); 
gcnames_ub_r = cat(1, gcnames_ub, gcnames_ra); 
gcnames_l = cat(1, gcnames, gcnames_ub_l);
gcnames_r = cat(1, gcnames, gcnames_ub_r);

segmnames_ub_l = cat(1, segmnames_ub, segmnames_la); 
segmnames_ub_r = cat(1, segmnames_ub, segmnames_ra); 
segmnames_l = cat(1, segmnames, segmnames_ub_l);
segmnames_r = cat(1, segmnames, segmnames_ub_r);
...
tmleft.p0 = p0_l;
tmleft.jcs = jc_l;
tmleft.gcnames = gcnames_l;
tmleft.segm_names = segmnames_l;
tmleft.CoM = CoM_l;

tmright.twists = tws_r;
tmright.p0 = p0_r;
tmright.jcs = jc_r;
tmright.gcnames = gcnames_r;
tmright.segm_names = segmnames_r;
tmright.CoM = CoM_r;
