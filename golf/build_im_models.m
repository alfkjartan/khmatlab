...
e_z = ejc_l - ghjc_l; % Local z-axis axially from shoulder
...
luarm.states = {'left shoulder x', 0.1050, ...
		...
		'left shoulder flex (ax elbow lat to elbow med)', 0.6668, ...
                luarm.localframe(1:3, 1)                
		'left shoulder add (ax ant to post)', 0.7578, ...
                luarm.localframe(1:3, 2)                
		'left shoulder rotation (ax dist)', 0.5297, ...
                luarm.localframe(1:3, 3)};
...
llarm.localframe = cat(1, cat(2, e_x, e_y, e_z, ejc_l),...
		      [0 0 0 1]);
llarm.dof = {[1 3], []};
llarm.states = {'left elbow flexion (ax lat to med)', 0.5708, ...
                llarm.localframe(1:3, 1)
                'left elbow rotation (ax dist)', 0.6794, ...
...
e_z = mid_mp_l - wjc_l;
e_z = e_z / norm(e_z);
e_x = mp_2_l - mp_5_l; % Local x-axis left-right given by
                       % MP markers
e_x = e_x / norm(e_x);
...
lhand.localframe = cat(1, cat(2, e_x, e_y, e_z, wjc_l),...
		      [0 0 0 1]);
lhand.dof = {[1 2], []};
lhand.states = {'left wrist ext/flex (ax mp5 to mp2)', 0.5369, ...
                lhand.localframe(1:3, 1)
	      'left wrist abduction (ax top to palm)', 0.8487, ...
...
e_z = ejc_r - ghjc_r; % Local z-axis axially from shoulder
                      % joint to elbow joint
e_z = e_z / norm(e_z);
...
ruarm.states = {'right shoulder x', 0.2678,...
	       ...
	       'right shoulder flex (ax elbow med to elbow lat)', 0.5174, ...
                ruarm.localframe(1:3, 1)
	       'right shoulder abd (ax ant to post)', 0.3660, ...    % Note definition of positive oposite from left side
                ruarm.localframe(1:3, 2)
	       'right shoulder rotation (ax dist)', 1, ...
                ruarm.localframe(1:3, 3)
               };
...
rlarm.localframe = cat(1, cat(2, e_x, e_y, e_z, ejc_r),...
		      [0 0 0 1]);
rlarm.dof = {[1 3], []};
rlarm.states = {'right elbow flexion (ax med to lat)', 0.7490, ...
                rlarm.localframe(1:3, 1)
	      'right elbow rotation (ax dist)', 0.2101, ...
...
e_z = mid_mp_r - wjc_r;
e_z = e_z / norm(e_z);
e_x = mp_5_r - mp_2_r; % Local x-axis left-right given by
                       % MP markers
e_x = e_x / norm(e_x);
% For the hand, adjust z-direction, not x-axis.
e_z = e_z - (e_z'*e_x)*e_x; 
...
rhand.localframe = cat(1, cat(2, e_x, e_y, e_z, wjc_r),...
		      [0 0 0 1]);
rhand.dof = {[1 2], []};
rhand.states = {'right wrist ext/flex (ax mp2 to mp5)', 0.9170, ...
                rhand.localframe(1:3, 1)
	      'right wrist abduction (ax top to palm)', 0.5507, ...
...
pelvis.states = {'pelvis x (ax left to right)', 0.0821, ...
                pelvis.localframe(1:3, 1)                 
		 'pelvis y (ax post to ant)', 0.0640, ...
                pelvis.localframe(1:3, 2)                 
		 'pelvis z (ax inf to sup)', 0.0770, ...
                pelvis.localframe(1:3, 3)                 
		 'pelvis tilt (ax left to right)', 0.1648, ...
                pelvis.localframe(1:3, 1)                 
		 'pelvis obliqueity (ax post to ant)', 0.2129, ...
                pelvis.localframe(1:3, 2)                 
		 'pelvis rotation (ax inf to sup)', 0.3120, ...
                pelvis.localframe(1:3, 3)};
...
trunk.states = {'trunk tilt (ax post to ant)', 0.2541, ...
                trunk.localframe(1:3, 2)                
	        'trunk obliquety (ax left to right)', 0.2468, ...
                trunk.localframe(1:3, 1)                
	        'trunk rotation (ax inf to sup)', 0.3226, ...
