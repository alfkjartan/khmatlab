function [theta,flag]=g2var(twists,g)
% function [theta,flag]=g2var(tws,g)
...
switch ntw
  ...
  case 3
    ...
    p=intersection(twists{1:2}) + 10*vwq(4:6,3);
    p=[p;1];
    q=g*p;
    if (norm(p-q)<tol)
      theta(1:2)=0;
    ...
    if ~theta(1:2)
      % Unable to solve for the first two angles. Do not attempt to
      % solve for the third.
      theta(3) = 0;
    else
      ...
      p=vwq(7:9,3)+U(:,3);
      p=[p;1];
      g1g2= expm( twists{1}*theta(1) )*expm( twists{2}*theta(2) );
      q=ginv(g1g2)*g*p;
      if (norm(p-q)<tol)
	theta(3)=0;
      else
  ...
  case 4
    ...
    p=vwq(7:9,4)+U(:,3);
    p=[p;1];
    g2g3=expr(twists{2}*theta(2))*expr(twists{3}*theta(3));
    q=inv(g1g2)*gg*p;
    if (norm(p-q)<tol)
      theta(4)=0;
    else    
  ...
  case 5
    ...
    g1g2=expr(twists{1}*theta(1))*expr(twists{2}*theta(2));
    gg=inv(g1g2)*g;
    % Find the next three angles as above
    % Find a point on the third axes that is not on the other two.
    p=p(1:3) + vwq(4:6,5);
    ...
    p=vwq(7:9,5)+U(:,3);
    p=[p;1];
    g3g4=expr(twists{3}*theta(3))*expr(twists{4}*theta(4));
    q=inv(g3g4)*gg*p;
    if (norm(p-q)<tol)
      theta(5)=0;
    else
  ...
  case 6
   ...
   theta=zeros(6,1);
    ...
    g1g2g3 = ...
	expr(twists{1}*theta(1)) * expr(twists{2}*theta(2)) * ...
	expr(twists{3}*theta(3));
    gg=inv(g1g2g3)*g;
    
    % Find the next three angles as above
    % Find a point on the third axes that is not on the other two.
    theta(4:6) = g2var(twists(4:6), gg);
    ...
    p=vwq(7:9,6)+U(:,3);
    p=[p;1];
    g4g5=expr(twists{4}*theta(4))*expr(twists{5}*theta(5));
    q=inv(g4g5)*gg*p;
